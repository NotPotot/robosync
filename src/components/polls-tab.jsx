...
function CreatePollModal({ onClose, onSubmit }) {
  ...
  const handleSubmit = (e) => {
    ...
    onSubmit({ question: question.trim(), timeLeft: timeLimit, channel })
    onClose()
  }
          ...
          <div>
            ...
            <div className="flex gap-2">
              {["Yes", "No", "Maybe"].map((opt) => (
                <div key={opt} className="flex-1 bg-[#2a2a2a] text-[#666] text-sm rounded-xl px-3 py-2.5 text-center">
                  {opt}
                </div>
              ))}
            </div>
